...
func TestDecide_Table(t *testing.T) {
	tests := []struct {
		...
		{
			"cisco up in proxy mode does nothing (rides Cisco)",
			DecideInput{Mode: ModeProxy, PrevCisco: CiscoInactive, NewCisco: CiscoActive},
			DecisionResult{Actions: []Action{ActNone}, NextMode: ModeProxy},
		},
		// Rule (c): Cisco down while in proxy -> refresh.
		{
			"cisco down in proxy refreshes upstream",
			DecideInput{Mode: ModeProxy, PrevCisco: CiscoActive, NewCisco: CiscoInactive},
			DecisionResult{Actions: []Action{ActRefreshProxy}, NextMode: ModeProxy},
		},
		// Startup: any transition involving Unknown is inert.
		{
			"initial unknown->active is inert",
			DecideInput{Mode: ModeProxy, PrevCisco: CiscoUnknown, NewCisco: CiscoActive},
			DecisionResult{Actions: []Action{ActNone}, NextMode: ModeProxy},
		},
		{
			...
			for _, nc := range states {
				for _, it := range intents {
					for _, pf := range []bool{false, true} {
						in := DecideInput{Mode: m, PrevCisco: pc, NewCisco: nc, Intent: it, PhysIfaceChanged: pf}
						a := Decide(in)
						b := Decide(in)
