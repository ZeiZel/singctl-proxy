...
// Selecting the Соединения section in the sidebar surfaces live connections +
// server latency as a content preview WITHOUT opening the full `c` overlay, so
// the dashboard shows the functionality inline.
func TestDashboard_ShowsConnectionsPanel(t *testing.T) {
	m := newWithCaps(&fakeBackend{}, nil, asciiCaps()).WithLoadedProfile()
	m.mode = RunProxy
	m.section = navConns // highlight Соединения → its preview fills the content pane
	...
	if m.ShowingConns() {
		t.Fatal("connections must show inline in the content pane, not via the overlay")
	}
	out := m.View()
	for _, want := range []string{"Соединения", "codex", "api.openai.com:443", "proxy-0", "42ms"} {
		if !strings.Contains(out, want) {
			t.Errorf("dashboard content missing %q:\n%s", want, out)
		}
	}
}
...
// At compact heights the sidebar dashboard still renders the nav list and never
// overflows the terminal height — the responsive contract for a short window.
func TestDashboard_CompactCollapsesToSummary(t *testing.T) {
	...
	if !strings.Contains(out, "Соединения") {
		t.Errorf("compact dashboard should still show the Соединения nav item:\n%s", out)
	}
